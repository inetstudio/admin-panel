...
<div class="form-group row @if ($errors->has($transformName)){!! "has-error" !!}@endif">
    ...
    <div class="col-sm-10">
        ...
        @foreach ($attributes['radios'] as $radio)
            <div><label> {!! Form::radio($name, $radio['value'], ($value === null && $loop->first || $radio['value'] == $value || $radio['value'] === old($transformName)) ? true : false, (isset($radio['options'])) ? $radio['options'] : []) !!} {{ $radio['label'] }} </label></div>
        @endforeach
